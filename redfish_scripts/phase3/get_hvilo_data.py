...
class Hardware(object):
    ...
    def create_process(self):
        ...
        json.dump(d, file('/app/monitor/ilo_scripts/phase3/output_json/'+cloud_json, 'w'))
        failed_data['hosts'] = {"cloud_name":self.cloud_name,"data":failed_host}
        json.dump(failed_data, file('/app/monitor/ilo_scripts/phase3/failed_host/'+cloud_json, 'w'))   
        json.dump(timeout_host, file('/app/monitor/ilo_scripts/phase3/failed_host/'+'timeout_'+cloud_json, 'w'))   
        pid= os.getpid()
    ...
    def getdiskbasichealth(self,redfishobject,hostname):
        basic_disk = {}
        disk_health = []
        response = redfishobject.get("/redfish/v1/Systems/1/SmartStorage/ArrayControllers/", None)
        if response.dict['Members@odata.count'] == 0:
        ...
        basic_disk['diskhealth'] = disk_health
        return basic_disk
    ...
    def getnetworkhealth(self,redfishobject,hostname):
        network_health = []
        network_health_dict = {}
        network_data = redfishobject.get('/redfish/v1/Systems/1/NetworkAdapters/', None)
        if network_data.dict['Members@odata.count'] <= 0:
            print("no network found for the host",hostname)
            return False
        else:
            output = network_data.dict['Members']
            for i in output:
                print("fetching network %s for the hostname %s"%(i,hostname))
                array_control = i.get('@odata.id')
                res = redfishobject.get(array_control)
                network = res.dict
                network_health.append(network.get('Status'))
    ...
    def getsystemhealth(self,redfishobject,res,hostname,failed_host):
        ...
        try:
            ...
            power_data = redfishobject.get("/redfish/v1/Chassis/1/Power/")
            ...
            if not system_data_dict['HostName']:
                ...
                try:
                    ...
                    data[t_hostname] = {"Bios_version":system_data_dict.get('BiosVersion'),"Power_state":system_data_dict.get('PowerState'),
                                        ...
                                        #"Power": [x.get('Status') for x in power_data.dict.get('PowerSupplies')],
                                        "Power": power_data.dict,
                                        "Temperature": thermal_data.dict.get('Temperatures')[0].get('Status'),
                                        "Network": self.getnetworkhealth(redfishobject,t_hostname),
                                        "Disk_Health":{"basic":self.getdiskbasichealth(redfishobject,t_hostname),
                                        "physical_disk":self.getdiskhealth(redfishobject,t_hostname,disk_type="physical"),
                ...
                try:
                    ...
                    data[system_data_dict['HostName']] = {"Bios_version":system_data_dict.get('BiosVersion'),"Power_state":system_data_dict.get('PowerState'),
                                                          "Processors":[system_data_dict.get('ProcessorSummary')],"manufacturer":system_data_dict.get('Manufacturer'),
                                                          "model":system_data_dict.get('Model'),"health":system_data_dict.get('Status'),
                                                          "Memory_status":system_data_dict.get('MemorySummary'),
                                                          #"Power": [x.get('Status') for x in power_data.dict.get('PowerSupplies')],
                                                          "Power": power_data.dict,
                                                          "Temperature": thermal_data.dict.get('Temperatures')[0].get('Status'),
                                                          "Network": self.getnetworkhealth(redfishobject,system_data_dict['HostName']),
                                                          "Fans":thermal_data.dict.get('Fans'),"ilo_version":firmware_version.dict["Current"]["SystemBMC"][0]["VersionString"],
                                                          "Disk_Health":{"basic":self.getdiskbasichealth(redfishobject,system_data_dict['HostName']),
                                                          "physical_disk":self.getdiskhealth(redfishobject,system_data_dict['HostName'],disk_type="physical"),
