    ...
    if 'health' in v.keys():
        if (v['health']['Health'] == 'Critical') or (v['health']['Health'] == 'Warning'):
            ...
            if 'prod.walmart.com' not in k:
                ...
                error_component = {'cloud':str(cloud_name),'hvname':str(hv_name),'hvhealth':str(v['health']['Health']),'critical_component':failure_compnent}
            else:
                error_component = {'cloud':str(cloud_name),'hvname':str(k),'hvhealth':str(v['health']['Health']),'critical_component':failure_compnent}
...
def main():
    ...
    json_file = [x for x in glob.glob('dfw3.json')]
