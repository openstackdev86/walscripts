...
def get_ilo_metrics():
    ...
    login_account = "admin1"
    login_password = "walmart1234"
    final_result = []
    fail_disk = []
    ok_disk= []
    global REST_OBJ
    ...
    for n in iLO_host:
        ...
        """
        path = 'redfish/v1/Managers/1/Actions/Oem/ManagerConfig.Reset'
        out = REST_OBJ.post(path=path)
       # body['ResetType']= "Off"
        ...
        # adding_user 
        #delete_ilo_account(REST_OBJ)
        #add_user = add_ilo_account(REST_OBJ)
        #power_data = power(REST_OBJ)
...
def power(REST_OBJ):
    ...
    out = '/redfish/v1/Systems/1/Actions/ComputerSystem.Reset'
    body = {}
    #body["Action"] = "Reset"
    ...
    #body["ResetType"] = "PushPowerButton"
    #body["ResetType"] = "On"
    res = REST_OBJ.post(path=out,body=body)
    #print data
...
def delete_ilo_account(obj):
    ...
    for user in del_user.get("Members"):
        get_user = obj.get(user.get('@odata.id')).dict.get('UserName')
        if get_user == "testing":
            obj.delete(path=user.get('@odata.id'))
    #check_existing_user(obj,resp,"testing") 
     
def add_ilo_account(obj):
    try:
        resp = REST_OBJ.get('/redfish/v1/AccountService/')
        body = {"UserName":"testing","Password":"testing123","RoleId":"Admin","Enabled":True}
        pdb.set_trace()
